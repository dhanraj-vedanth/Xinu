...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	// Paging stuff
	/*
	1. Initialize backing stores
	2. Initialize frames 
	3. Create a page dir and page table 
	*/
	...
	int frm_no = 0; 
	for (i;i<4;i++){
		// Need to find 4 free frames now
		int retr = get_frm(&frm_no);
		if (retr == SYSERR){
			// Handle page replacement here I guess?
			return SYSERR;
		}
		frm_tab[frm_no].fr_status = FRM_MAPPED;
		frm_tab[frm_no].fr_type = FR_TBL;
		frm_tab[frm_no].fr_pid = NULLPROC; 
		pgptr = (FRAME0 + frm_no) * NBPG;
		kprintf("Global frame number->%d\n",FRAME0+frm_no);
		j = 0;
		for(j;j<1024;j++){
			pgptr->pt_write = 1;
			pgptr->pt_pres = 1;
			...
			pgptr->pt_global = 1;
			pgptr->pt_avail = 0;
			// Not correct the bottom one.
			// pgptr->pt_base = 0;
			pgptr->pt_base = (i * 1024) + j;
			pgptr += 1;
		}
	}
	create_pgd(NULLPROC);
