...
#define PRLWAIT     '\011'		/* process lock state */
...
struct	pentry	{
	...
	//PA2 stuff 
	int lockstate[NLOCKS]; 		/* Semaphore state [deleted(1)/read or write]*/
	int wait_times; 	/* Wait times for each of the semaphore for a process*/
	int lockprio;
	int origprio;
	int pinh; /* Inherited prio */
	int blkid; /* In which sem wait queue am I in? One of the NLOCKS* (-1--> Not in any wait queue)*/
